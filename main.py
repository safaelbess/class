...
class Bank:
  ...
  def __init__(self, name, location):
    ...
    self.bank_name = name
    self.bank_location = location

  def __str__(self):
    return f' welcome to our {self.bank_name } bank , the bank is located in {self.bank_location} '
...
def __str__(self):
  return f'the client name is {self.client_name} and the client age is {self.age}, the client address is {self.client_address}, and the client id is {self.client_id}'


class Account(Person, Bank):

  def __init__(self, number, balance, client_name, age, client_address,
               client_id, bank_name, bank_location):
    self.withdraw = withdraw
    self.account_number = number
    self.balance = balance
    Person.__init__(self, client_name, age, client_address, client_id)
    Bank.__init__(self, bank_name, bank_location)

  def __str__(self):
    return f' hello {self.client_name} , your balance is {self.balance}  ,your address is {self.client_address}, your id is {self.client_id} and  your bank bank_name is {self.client_name}'


def withdraw(self, withdraw=int(input('enter the withdraw amount'))):
  self.balance = self.balance - withdraw
  return f' the new balance is {self.balance}'


def deposite(self, deposite=int(input('enter the amount you want to deposit'))):
  self.balance = self.balance + deposite
  return f'the new balance after deposit is {self.balance}'


accounts = []
while True:
  account1 = Account('12345', 34455, 'ali alqadi', 40, 'amman', 5566,
                     'islamic ', 'amman')

  print(account1)

  account_id = random.randrange(1000000, 9000000)

  accounts.append(account_id)

  print(accounts)

  if input('is there any other account will be added ?' == 'no'):
    break
